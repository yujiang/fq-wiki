    ...
    <div v-else>
      <div v-for="(entry0, scene) in sortedEntries" :key="scene" class="area-row">
        ...
        <div v-for="(entry, id) in entry0" :key="id" class="skill-row">
            ...
            <div class="skills">
            <SkillGrid :skills="entry" :cols="3" :rows="entry.length > 3 ? 2 : 1" />
            </div>
        </div>
        </div>
      </div>
...
.skill-row { display:flex; gap:1rem; align-items:center }

.npc-row { display:flex; gap:0.5rem; flex-wrap:wrap }
</style>
